...
import { guardiaAutenticazione, guardiaSoloNativo, guardiaLanding, guardiaTutorial } from './core/guards/guards';
import { ComponenteShellApp } from './shell/app-shell.component';

export const routes: Routes = [
  // Tutorial di primo avvio. La radice reindirizza qui; guardiaTutorial o lo mostra (primissimo
  ...
  { path: 'onboarding', loadComponent: () => import('./components/onboarding/onboarding.component').then(m => m.ComponenteOnboarding), canActivate: [guardiaTutorial] },
  { path: 'auth', loadComponent: () => import('./pages/auth/auth.page').then(m => m.PaginaAutenticazione) },
  { path: 'benvenuto', loadComponent: () => import('./pages/landing/landing.page').then(m => m.PaginaBenvenuto), canActivate: [guardiaLanding] },
  // La radice nuda va a /onboarding, che guardiaTutorial risolve: il primissimo avvio mostra
  ...
  {
    path: '', component: ComponenteShellApp, canActivate: [guardiaAutenticazione],
    children: [
      ...
      { path: 'camera/:id_evento',   loadComponent: () => import('./pages/camera/camera.page').then(m => m.PaginaFotocamera), canActivate: [guardiaSoloNativo] },
